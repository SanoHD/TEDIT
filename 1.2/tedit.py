...
syntax = {
    "py": {
        "words": {
                "yellow":["continue","pass","while","True","False","try","except","if","elif","else","with","as","is","for","break","import"],
    ...
    while True:
        ...
        for ln, line in enumerate(text):
            ...
            if filename.split(".")[-1] in syntax: # Test if file-type is available as syntax-highlighting
                ...
                """
                #if comment_block:
                #    line = colored(line,comment_block_color)
                if line.strip()[:len(comment_symbol)] == comment_symbol:
